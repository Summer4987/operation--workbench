...
run_budget_step "饿了么${PERIOD}预算" "${ELEME_BUDGET_TIMEOUT_SECONDS:-1800}" "${BUDGET_STEP_RETRIES:-2}" /bin/zsh "$ELEME_RUNNER" --time "$TIME_POINT" --mode "$MODE" --limit "$LIMIT" || true

echo
if [[ "$MODE" == "commit" ]]; then
  echo "执行美团${PERIOD}预算提交前预检..."
  if run_budget_step "美团${PERIOD}预算提交前预检" "${MEITUAN_BUDGET_PREFLIGHT_TIMEOUT_SECONDS:-900}" 1 "$REPORT_PYTHON" store-inspection/meituan_budget_cdp.py --period "$PERIOD" --mode preview --limit "$LIMIT" --preflight; then
    echo
