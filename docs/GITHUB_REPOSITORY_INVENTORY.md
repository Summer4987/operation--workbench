...
Mac mini 是生产环境，默认从 GitHub 拉取已验证代码运行。遇到生产现场 bug 时，允许在 Mac mini 上直接修复，但修复后的有效代码、脚本、看板或配置模板改动，完成验证后必须当天提交并推送到 GitHub。

生产修复提交仍然必须遵守仓库归档规则：

- 不提交浏览器 profile、Cookie、日志、下载数据、运行结果和密钥。
- 不提交临时截图、调试产物和名字带 ` 2` 的重复副本。
- 不把无关改动和本次修复混在同一个提交里。
- 如果无法推送，必须记录阻塞原因和下一步处理方式。
