...
- “Hermes 控制台 / agent 控制台 / 后台 / 透明化” 指未来要建设的 Hermes 可视化控制台，用于查看在线状态、最近指令、任务结果、失败原因、文件产物、简称表和重启状态。
...
- 推广助理：默认只做预算/出价建议、审批队列和预览；没有明确确认，不真实调整预算或出价。
- 控制台助手：未来要支持 Hermes 控制台，让用户不用终端也能查看在线状态、任务状态、失败原因和产物。

## 自动化任务通知规则

- 每次自动化任务结束后，应尽量向用户汇报成功或失败。
- 成功通知要短：说明任务名、成功、关键产物或时间。
- 失败通知要有用：说明失败环节、最可能原因、是否已补跑、下一步建议。
