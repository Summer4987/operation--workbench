...
- “出价 / 推广出价 / 点金出价” 遵循直接改价规则：用户明确说“美团/饿了么 + 门店 + 点金/关键词/推广出价 + 调到 X 元”时，直接调用 `scripts/promo_bid_direct_request.py --execute <用户原文>`；如果缺平台、门店或目标价格，只追问缺失字段。不要再要求二次确认。当前若真实执行器未接入，必须明确回复“已识别但缺执行器”，不能假装已改。
